...
load('RNN40v30_cv.mat')
load('RNN40v30_wt.mat')
...
for v=1:length(ktest)
    ...
    for j=1:30 % 30 conditions
        ...
        Test3Output.ActState(v).condition(j).OUT_desired=ActState(k).Ycho(j);
...
for v=1:length(ktest1)
    ...
    for j=1:30 % 30 conditions
        ...
        CV1test(1).set(v).cdtion(j).OUT_desired=ActState(k).Ycho(j);
