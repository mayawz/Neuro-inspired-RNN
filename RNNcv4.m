...
ktrain=crosVal(2).KTrain;
ktest=crosVal(2).KTest;
ktest1=crosVal(2).KTest1;
ktest2=crosVal(2).KTest2;
...
for i=1:epochs
    ...
    for v=1:t1
        ...
        for j=1:30 % 30 conditions
            ...
            if i > 1500
            CV4.epoc(i).set(v).cdtion(j).trainOUT=output_temp;
            CV4.epoc(i).set(v).cdtion(j).trainTAR=output_desired;
            CV4.epoc(i).set(v).cdtion(j).H=h_temp;
            CV4.epoc(i).set(v).cdtion(j).H1=hidden_temp;
            end
            y_tar=output_desired;
            y_out=output_temp;
            
            CV4.epoc(i).set(v).cdtion(j).logL=y_tar*log(y_out)+(1-y_tar)*log(1-y_out);
        ...
        disp(['logL = ' num2str(CV4.epoc(i).set(v).cdtion(j).logL)])
    end  % end of Ktrain sets of activation states
    
    if ismember(i,saveWtEpo)
        CV4.midWt(i/50).WT=wieght;
    end
